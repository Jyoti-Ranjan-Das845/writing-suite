...
If a consequential point cannot be established from the repository or existing context, flag it in the schema part's `<notes>` field and set `<status>Blocked</status>` until it is resolved.
