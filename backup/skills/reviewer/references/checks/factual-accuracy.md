...
Fidelity to a supplied or grounded source does not by itself establish correctness. Conversely, content is not factually wrong merely because it uses authorized ordinary domain knowledge that was not copied from `grounding.md`.
...
A factual or technical statement can fail this check even when it reproduces `grounding.md` exactly, if available evidence or direct inspection establishes that the underlying claim or implementation is materially incorrect.

When `authority.md` permits ordinary domain knowledge, technical correction, or safe inference, evaluate the resulting content for correctness rather than rejecting it solely because it originated outside `grounding.md`.

Check the artifact against:

1. `requirements.md` — what correctness level the artifact requires;
2. `grounding.md` — established factual and source basis;
3. `authority.md` — permitted augmentation and correction boundary, not evidence that the resulting factual content is true;
...
## FA-01 — Grounding informs factual verification

Use grounded content to identify the factual claim, its intended source basis, and any evidence that requires verification.

Grounding identifies the intended factual basis, but is not itself a guarantee that the grounded claim is correct. If the Writer faithfully reproduced a grounded claim that is independently shown to be wrong, record **Factual accuracy**, not **Grounding fidelity**. If the Writer changed a correctly grounded fact, record **Grounding fidelity**. If the Writer added content outside grounding but within `authority.md`, evaluate its truth normally; do not flag it solely for being outside grounding.
...
Ordinary domain knowledge or safe inference permitted by `authority.md` must still be expressed at a certainty level justified by the evidence and nature of the claim. Authorization does not convert inference into fact.
...
Use `grounding.md` as the primary established source basis when the task has already established its factual source material, but distinguish fidelity to that basis from independent correctness.

This prevents the Reviewer from silently introducing a different factual basis.

Do not duplicate a Grounding fidelity finding merely because the same claim appears in `grounding.md`. If the Writer changed a correctly grounded fact, record Grounding fidelity. Record Factual accuracy only when available evidence independently shows that the grounded or final factual claim is wrong or unsupported.
...
- return to Content Grounding for evidence;
- or return to the Authoring Contract when correction authority, requester input, or verification strategy must change.
...
- `Suggested direction`: state whether the Writer should correct, qualify, remove, verify, return to Content Grounding, or return to the Authoring Contract when authority or resolution must change
